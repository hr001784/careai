...
async def voice_endpoint(
    ...
    # Get query params manually for reliability
    ...
    # Use a fresh DB session for this websocket connection
    from app.models.database import AsyncSessionLocal
    
    try:
        # Don't call memory.connect() here, it's already handled in get_memory()
        # and we don't want to fail the whole connection if Redis is just temporarily down
        pass
    ...
    try:
        while True:
            ...
            # STT and logic inside the loop
             async with AsyncSessionLocal() as db:
                 # We pass the db session to any tool that needs it later
